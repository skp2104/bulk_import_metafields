...
const apiCall = async (query_data) => {
  const url =
    'https://dev-store-data.myshopify.com/admin/api/2021-07/graphql.json';
  const headers = {
...
const importquery = async () => {
  ...
  console.log(
    urlid.data.data.stagedUploadsCreate.stagedTargets[0].parameters[4].value +
      'x-goog-date....'
  );
  console.log(
    urlid.data.data.stagedUploadsCreate.stagedTargets[0].parameters[5].value +
      'x-goog-credential....'
  );
  console.log(
    urlid.data.data.stagedUploadsCreate.stagedTargets[0].parameters[6].value +
      'x-goog-algorithm...'
  );
